...
bool HPSListViewDelegate::editorEvent ( QEvent * event, QAbstractItemModel * model, const QStyleOptionViewItem & option, const QModelIndex & index ) {
    //qDebug() << "nmuh" << *check;
    if( event->type() == QEvent::MouseButtonRelease && !(*mCheck)){
        QMouseEvent *mouseEvent = static_cast<QMouseEvent *>(event);
        if(mouseEvent->button()==Qt::LeftButton){
            qDebug()<< "MouseButtonLeftRelease";
            QRect re(option.rect.left()+mSize-30,option.rect.top(),
                     option.rect.width()-mSize+30,option.rect.height()-mSize+30);
